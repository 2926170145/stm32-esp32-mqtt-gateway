...
void MyI2C_W_SCL(uint8_t BitValue)
{
	...
	Delay_us(10);												//I2C标准模式要求时钟频率 ≤ 100kHz，每个位周期 ≥ 10μs,延时10us，防止时序频率超过要求
}
...
void MyI2C_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PA8和PA9引脚初始化为开漏输出
	
	/*设置默认电平*/
	GPIO_SetBits(GPIOA, GPIO_Pin_8 | GPIO_Pin_9);			//设置PA8和PA9引脚初始化后默认为高电平（释放总线状态）
}
